...
@section('content')
    <div class="container mx-auto">
        <h2 class="mb-2 text-xl">Edit {{ $project->name }}</h2>
        <form action="{{ route('projects::store', [$project]) }}" method="post" class="border border-gray-400 bg-white lg:w-1/3 w-full rounded p-3">
            @csrf
            @method('put')
